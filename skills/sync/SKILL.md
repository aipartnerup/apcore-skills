...
| `--scope` | `all` | Which group: `core` (apcore SDKs), `mcp` (apcore-mcp bridges), `all` |
...
- `impl_repos[]` — the implementation repo paths
...
From `spec_api` and all `api_summaries`, construct a union of all symbols. Apply canonical name normalization (convert all names to `snake_case`) for matching:
...
- `verified_api` — the confirmed-correct API surface (spec + confirmed implementations)
- `checklist` — every item with its PASS/FAIL/WARN status
- `findings` — structured list of all failures with severity

This `verified_api` becomes the input truth for Phase B.
